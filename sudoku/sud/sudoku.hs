...
example =
    Sudoku
      [ [Just 4, Nothing, Nothing,Nothing,Nothing,Nothing,Nothing,Just 6,Nothing]
      , [Nothing,Just 6, Nothing,Nothing,Just 3,Nothing,Just 7, Nothing,Just 1]
      , [Just 8,Nothing, Nothing,Nothing, Nothing, Just 4, Just 2, Nothing,Just 3]
      , [Just 5,Nothing,Just 8,Nothing,Nothing,Nothing,Just 3, Just 2, Nothing]
      , [Just 2, Nothing,Nothing,Nothing, Nothing,Nothing, Nothing,Just 7, Nothing]
      , [Nothing, Nothing,Nothing, Nothing, Just 1,Nothing,Nothing,Just 8,Nothing]
      , [Nothing,Nothing, Nothing, Nothing,Nothing,Nothing,Nothing,Nothing,Nothing]
      , [Nothing,Just 4, Just 5, Nothing,Just 7,Just 8,Nothing,Just 3, Nothing]
      , [Just 6,Nothing, Nothing,Nothing,Nothing,Just 3, Just 5, Nothing,Nothing]
      ]
...
prop_update s c x = x == (head (take 1 (drop (snd c) (concat(take 1 (drop (fst c) (rows (update s c x))))))))
...
solve' s 
          ...
          |  otherwise = solve' (head(filter isOkay (fill3 [s]  (blankCoord s))))
...
fill2 :: [Sudoku] -> [(Int,Int)] -> [Sudoku]
fill2 s [] = s
fill2 s (c:cs) = fill2 (filter isOkay (fill2' s c)) cs

fill2' :: [Sudoku] -> (Int,Int) -> [Sudoku]
fill2' [] c = []
fill2' (s:ss) c =  ([update s c x | x <- (filter (/= Nothing) sudokuElem) ]) ++ (fill2' ss c)

fill3 :: [Sudoku] -> [(Int,Int)] -> [Sudoku]
fill3 s [] = s
fill3 s (c:cs) = fill3 (filter isOkay (fill3' s c)) cs

fill3' :: [Sudoku] -> (Int,Int) -> [Sudoku]
fill3' [] c = []
fill3' (s:ss) c =  ([update s c x | x <- (filter (`notElem` (currentRowElem s c)) sudokuElem) ]) ++ (fill3' ss c)
...
s1 `isSolutionOf` s2 = fromJust (solve s2) == s1

...
f9 = fill9 (fill9 [example] (nineFstBlanks example)) (nineFstBlanks fill9 [example] (nineFstBlanks example))

-- function that is supposed to
fill9 :: [Sudoku] -> [(Int,Int)] -> [Sudoku]
fill9 s [] = s
fill9 s (c:cs) = fill9 (filter isOkay (fill9' s c)) cs

fill9' :: [Sudoku] -> (Int,Int) -> [Sudoku]
fill9' [] c = []
fill9' (s:ss) c =  ([update s c x | x <- (filter (/= Nothing) sudokuElem) ]) ++ (fill9' ss c)
